...
本版启用 **AI 编写 JavaScript → 完整数据执行 → 结果预览 → 原稿写入与历史撤销**。新生成流程不再组合 Transform/Renderer DSL。旧规则保留执行兼容。

请先阅读 [JavaScript 版本验收](JAVASCRIPT_VALIDATION.md)。文档后续“能力组合”章节是旧规则的兼容性说明，不是新版默认生成链路。
