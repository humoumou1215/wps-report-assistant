...
新版计算／展示由 AI 编写 JavaScript，运行后返回通用变量或 text/table 写入计划。宿主能力注册仍负责定位、原稿写入和恢复。详见 JAVASCRIPT_VALIDATION.md。
...
- 表格：读取最多 5000 行；一次写入最多 2000 个单元格；输出可按 `mergeBy` 将相邻且字段值相同的结果行纵向合并；已有合并区域仍拒绝写入。公式、值、数字格式和已支持的字体／对齐属性进入快照。
